...
export function Phase2Adjudication(props: Props) {
  ...
  return (
    <>
      ...
      {state.compareSelected.length > 0 && <section className="compare-panel" aria-label="Compare selected proxies"><h3>Compare selected ({state.compareSelected.length})</h3><label>Amount<input className="ccc-input" value={state.compare.amount} onChange={(e) => props.dispatch({ type: 'setCompareField', field: 'amount', value: e.target.value })} /></label><button onClick={props.runCompare} disabled={state.compareSelected.length < 2}>Compare selected</button>{state.compare.result && <p className="result">{state.compare.result}</p>}{state.compare.error && <p className="error">{state.compare.error}</p>}</section>}
      {props.recommendation && <section className="context-card"><h3>Original vs adjusted</h3><p className="helper">{state.calculation?.camelValue.toFixed(2)} → {props.recommendation.adjustedCamelValue.toFixed(2)} camels ({props.recommendation.regionFactor.toFixed(2)}x region · {props.recommendation.traitFactor.toFixed(2)}x traits)</p><button className="ccc-button-primary cta-primary" onClick={props.finalizeBid}>Finalize Bid</button><button className="cta-secondary" onClick={props.resetToOriginalBid}>Reset to Original</button></section>}
